...
uint8_t *adc1_data = scope_ch1_data;
uint8_t *adc2_data = scope_ch2_data;  
    ...
    if (ofs == REG_PIO_DATA) {
        ...
        if (((reg & FPGA_CLOCK_PIN) == 0) && (value & FPGA_CLOCK_PIN))
        {
            ...
            case FPGA_COMMAND_WRITE:
                ...
                case 0x05: // Check Ready
					//pd->print_command = 0;
                    s->ptr = &s->ready;
                    break;
                    
                case 0x06: // Read Version
					//pd->print_command = 0;
                    s->ptr = version;
                    s->cnt = 2;
                    break;

                case 0x0A: // Check Buffer Full or Triggered
					//pd->print_command = 0;
                    s->ptr = &s->ready;
                    break;                    
                    
                case 0x0f: // Enable Trigger Mode
                    s->ptr = s->data;
                    break;
                ...
                case 0x28: // Short Time base mode
					s->ptr = s->data;
                    break;
                    
                case 0x38: // Set brightness
					//pd->print_command = 0;
					s->ptr = brightness;
                    s->cnt = 2;
                    break;

                case 0x41: // Read touch panel coordinates register address
					s->ptr = tp_coords;
                    s->cnt = 2;
                    break;

                case 0x64: // Read Parameters from the chip
					//pd->print_command = 0;
                    break;

                case 0x65: // Write Parameters to the chip
					//pd->print_command = 0;
                    break;

                case 0x66: // Start the process based on the mode
					//pd->print_command = 0;
                    if (s->cmd == 0x64) {
