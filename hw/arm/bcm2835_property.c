...
#define TYPE_BCM2835PROPERTY "bcm2835_property"
#define BCM2835PROPERTY(obj) \
    OBJECT_CHECK(bcm2835_property_state, (obj), TYPE_BCM2835PROPERTY)
...
static int bcm2835_property_init(SysBusDevice *sbd)
{
    ...
    bcm2835_property_state *s = BCM2835PROPERTY(dev);

    s->pending = 0;
    s->addr = 0;

    sysbus_init_irq(sbd, &s->mbox_irq);
    memory_region_init_io(&s->iomem, NULL, &bcm2835_property_ops, s,
        TYPE_BCM2835PROPERTY, 0x10);
