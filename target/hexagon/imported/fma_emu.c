...
static inline double dumb_fma(double a, double b, double c)
{
	return a * b + c;
}
...
/* Return an infinity with the same sign as a */
static inline hf_t infinite_hf_t(xf_t a)
{
	hf_t ret;
	ret.x.sign = a.sign;
	ret.x.exp = HF_INF_EXP;
	ret.x.mant = 0ULL;
	return ret;
}

/* Return a maximum finite value with the same sign as a */
static inline hf_t maxfinite_hf_t(xf_t a)
{
	hf_t ret;
	ret.x.sign = a.sign;
	ret.x.exp = HF_INF_EXP - 1;
	ret.x.mant = 0x03ffUL;
	return ret;
}

static inline hf_t f2hf_t(float in_f)
{
	hf_t ret;
	sf_t in;
	in.f = in_f;
	...
	ret.x.exp = in.x.exp - fSF_BIAS() + fHF_BIAS();
	return ret;
}
...
static inline double special_fma(df_t a, df_t b, df_t c)
{
	...
	/* If none of the above checks are true and C is a NaN, a NaN shall be returned */
	...
	/* We have checked for adding opposite-signed infinities.  Other infinities return
...
double internal_fmax(double a_in, double b_in, double c_in, int scale)
{
	...
	/* Note: extracting the mantissa into an int is multiplying by 2**52, so adjust here: */
...
double internal_mpyhh(double a_in, double b_in, unsigned long long int accumulated)
{
	...
	accumulated >>= 1;
	xf_init(&x);
	if (isz(a_in) || isnan(a_in) || isinf(a_in)) return internal_mpyhh_special(a_in,b_in);
	if (isz(b_in) || isnan(b_in) || isinf(b_in)) return internal_mpyhh_special(a_in,b_in);
